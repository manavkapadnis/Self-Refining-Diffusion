#!/bin/bash
#SBATCH --job-name=tpdm_only_lora_15
...
#SBATCH --output=tpdm_only_lora.out
#SBATCH --error=tpdm_only_lora.err
...
OUTPUT_DIR="/data/user_data/mkapadni/genai_project/outputs_tpdm_lora/$(date +'%Y-%m-%d')/$RUN_NAME"
...
python -m torch.distributed.run --nproc_per_node $NUM_GPUS --nnodes 1 --standalone \
    ...
    --output_dir "/data/user_data/mkapadni/genai_project/outputs_tpdm_lora/2025-04-27/training_diffusion_laion_2025-04-27/checkpoint-100" \
    --run_name $RUN_NAME \
