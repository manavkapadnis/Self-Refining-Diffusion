#!/bin/bash
#SBATCH --job-name=self_refine_lora
...
#SBATCH --output=self_refine_lora.out
#SBATCH --error=self_refine_lora.err
...
OUTPUT_DIR="/data/user_data/mkapadni/genai_project/outputs_self_refine/$(date +'%Y-%m-%d')/$RUN_NAME"

# Run training with LoRA
python -m torch.distributed.run --nproc_per_node $NUM_GPUS --nnodes 1 --standalone \
    main_diff_self_refine_trainer.py \
    ...
    --output_dir "/data/user_data/mkapadni/genai_project/outputs_self_refine/2025-04-27/training_diffusion_laion_2025-04-27/checkpoint-100" \
    --run_name "training_diffusion_laion_2025-04-27" \
